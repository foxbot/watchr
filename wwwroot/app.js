...
const Chat = {
    view() {
        return m('.chat', { onclick() { state0.increment() } }, 'the state he say:' + state0.data.v);
    },
    oninit(vnode0) {
        state0.socket.start();
    }
...
const Media = {
    view() {
        return m(".media", { onclick() { state3.increment() } }, [
            m("p", "media testlet - the state he say:" + state3.data.v),
            m("p", "the websocket is big: " + state3.socket.ws.readyState),
            m("p", "media url he say", state3.data.mediaUrl)
        ])
    }
}
;
